...
저는 관련 비디오를 랜덤정렬해서 비디오 페이지에 디스플레이하고 싶었습니다. 처음에는 sort() 함수를 이용해서 구현하려했었으나 방법이 떠오르지 않더군요. 그리고 검색을 해보니 [피셔예이츠 셔플 알고리즘(티 스토리에 정리)](https://taesung1993.tistory.com/54?category=872368)이라는 것을 발견했습니다. 따라서 이 알고리즘을 이용하여 원하는 기능을 구현할 수 있었습니다. 이 기능을 구현하는데는 많은 시간이 걸리지는 않았지만, 프로젝트에 처음으로 알고리즘을 이용해서 해결했다는 점에서 개인적으로 굉장히 의의가 있다고 생각했습니다.
